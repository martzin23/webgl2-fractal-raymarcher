...
Primarni alat koji je korišten za ovaj rad je biblioteka \emph{WebGL2}. To je varijanta popularne grafičke biblioteke OpenGL koja je prilagođena za rad na internetskom pregledniku i jeziku \emph{JavaScript}. Mogućnost korištenja grafičkih funkcija na internetskom pregledniku ima mnoge prednosti: omogućuje kompleksne vizualizacije unutar web stranica, izradu video igara koje su dostupne bez instaliravanja izvršne datoteke i generalni pristup procesorne moći grafićke kartice unutar JavaScript. \emph{WebGL2} nije sasvim identičan OpenGL, jedna bitna razlika je što ne omogućuje izradu računskih sjenčara (\emph{engl. compute shaders}). Takvi sjenčari su pogodni za iscrtavanje temeljenom zrakama kao praćenje sfera, no moguće je izvesti takvo iscrtavanje koristeći sjenčare fragmenta.
...
\begin{figure}[htb]
	...
	\caption{Skraćeni dijagram razreda}
	\label{slk:classes}
\end{figure}
...
Dizajn korisničkog sučelja je napravljen koristeći standardnih HTML elementa i CSS pravila, a funkcionalnost sučelja je ostvarena u JavaScript. Cilj sučelja je bilo omogućiti pristup što većem broju varijabla. Uz kretanje kroz virtualni prostor, ostvarena je mogućnost podešavanja načina prikaza i čak mijenjanje funkcija daljine koje se prikazuju. Tekst sučelja je na engleskom jeziku. Upravljanje varijablama je ostvareno raznim kontrolama kao gumbovi slajderi, unos brojeva, itd. Većinu elemenata imaju kratak opis koji se pojavi ako se kratko vrijeme ostavi miš iznad njih. Slike ispod su korisničko sučelje na stolnom računalu (slika \ref{slk:desktop}) i na mobilnom uređaju (slika \ref{slk:mobile}).

Sučelje je podijeljeno u četiri kartice: generalne postavke, postavke vezane uz praćenje sfere, unos funkcije daljine i informacije o kontrolama. U kartici za informacije o kontrolama jer samo tekstualni opis gumba na tipkovnici i njihovih pripadajućih funkcionalnosti. Generalne postavke uključuju porešavanje kamere, rezolucije i slikanje ekrana. Postavke za praćenje sfere su više tehničke, imaju najveći utjecaj na brzinu iscrtavanja i mogu upravljati oblikom tijela. Konačno, u zadnjoj kartici se može unijeti funkcija daljine. Postoje kratke naputnice za izradu funkcija i jedan primjer koda. 
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.38\linewidth]{images/mobile.png}
...
Funkcija daljine se unosi u tekstualni okvir u obliku izvornog programskog koda jezika \emph{GLSL}. \emph{GLSL} je progmramski jezik za sjenčare kojeg koristi \emph{WebGL2}. Funkcija daljine može koristiti druge funkcije u izvornom kodu koji se unese. Nakon unosa se treba kliknuti gumb za kompajliranje koda. Prvo se izbriše postojeći program koji \emph{WebGL2} koristi za iscrtavanje, zatim se uneseni izvorni kod u obliku teksta spoji zajedno s izvornim kodom ostatka sjenčara fragmenta. Konačno se novi spojeni izvorni kod pošalje grafičkoj kartici, kompajlira se i stvori se novi program za iscrtavanje. Ako dođe do greške pri kompajliranju (kao na primjer sintaksna greška unesenog koda), funkcija daljine će se zamijeniti zadanom defaultnom funkcijom i ispisati će se poruka koja daje informaciju o greški radi jednostavnijeg popravljanja koda.
